...
public class TransaccionalesController {
        ...
        @GetMapping("/generico")
        public ResponseEntity<TransactionalRequest> consultaGenerica() {
                return new ResponseEntity<>(TransactionalRequest.builder().operationId("192871271521892162761298712")
                                .category("7").causeCode("P").companyCode("2").contactMode("Email").documentType("DNI")
                                .documentNumber("10266305").embozo("121").option("ReimprimirTarjeta").origin("WEB")
                                .productCode("B").productNumber("4517610097274041").reasonCode("134")
                                .registerSector("SS").responsibleSector("951").user("43275857").build(), HttpStatus.OK);
        }
        ...
        public ResponseEntity<Transaccional> consultarEntes(@RequestParam("operationId") String operationId,
                ...
                if (authorityList != null && !authorityList.isEmpty()) {
                        transaccional = Transaccional.builder().authorities(authorityList).header(getSuccessResponse())
                                        .build();
                } else {
                        transaccional = Transaccional.builder().header(getNotFoundResponse()).build();
                }
        ...
        public ResponseEntity<Transaccional> consultaSucursales(@RequestParam("operationId") String operationId) {
                return new ResponseEntity<>(
                                Transaccional.builder().branches(getSucursales()).header(getSuccessResponse()).build(),
                                HttpStatus.OK);
        }
        ...
        private BaseResponse getInternalErrorResponse() {
                ErrorResponse errorResponse = ErrorResponse.builder().faultString("ServerException")
                                .detail("Internal Error.").faultCode("ESSR").build();
